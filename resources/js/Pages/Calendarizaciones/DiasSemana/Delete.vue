<template>
    <GenericModal :dataModal="dataModal" @cerrarModal="cerrarModal">
        <template #header>
            <h3 class="font-bold">Eliminar día de la semana</h3>
        </template>
        <template #content>
            <div class="d-flex align-items-center">
                <i class="pi pi-exclamation-triangle"/>&nbsp;
                <span>¿Seguro que desea eliminar el día de la semana <strong>{{ dataModal.dataRegistro?.nombre }}</strong>?</span>
            </div>
...
const eliminarRegistro = () => {
    form.delete(route("Calendarizaciones.DiasSemana.destroy", props.dataModal.dataRegistro), {
        onSuccess: () => {
            cerrarModal();
        },
    });
