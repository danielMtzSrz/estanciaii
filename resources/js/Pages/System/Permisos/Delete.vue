<template>
    <Dialog v-model:visible="dataModal.display" :breakpoints="{ '960px': '75vw', '640px': '100vw' }"
        ...
        <div class="flex justify-between mt-0 mb-2">
            <h3 class="font-bold">Eliminar permiso</h3>
            <button @click="cerrarModal()"
                class="rounded-full px-2 py-1 shadow-md  transition-all transform hover:shadow-xl hover:shadow-gray-700">
                <span class="p-dialog-header-close-icon pi pi-times"></span>
            </button>
        </div>
        ¿Seguro que desea eliminar el permiso <strong>{{ dataModal.dataPermiso.name }}</strong>?
...
export default {
    ...
    methods: {
        ...
        eliminarUsuario() {
            this.form.delete(route("permission.destroy", this.dataModal.dataPermiso), {
                onSuccess: () => {
                    this.cerrarModal();
                    this.form.reset();
                },
