                ...
                <div class="row p-fluid pt-4">
                    ...
                    <div class="col-sm-12">
                        <Dropdown 
                            ...
                            :data="dataTiposAnuncio"
                            textDropdown="nombre"
                            v-model="tipoAnuncioSeleccionado"
                        />
                    </div>
...
const dataTiposAnuncio = ref(null), tipoAnuncioSeleccionado = ref(null)

const imagenActual = ref(null)

const form = useForm({
    _method: null,
    tipo_anuncio_id : null,
    empresa_id : null,
    titulo : null,
    contenido : null,
    imagen : null
...
watch(() => props.dataModal, async (newVal) => {
    ruta.value = !newVal.dataRegistro ? 'anuncios.store' : 'anuncios.update'
    titulo.value = !newVal.dataRegistro ? 'Nuevo anuncio' : 'Actualizar anuncio'

    const dataEmpresasAxios = await axios.get(`/api/empresas`);
    dataEmpresas.value = dataEmpresasAxios.data;

    const dataTiposAnuncioAxios = await axios.get(`/api/tipos_anuncio`);
    dataTiposAnuncio.value = dataTiposAnuncioAxios.data;
})
...
watch(() => props.dataModal.dataRegistro, (newVal) => {
    ...
    empresaSeleccionada.value = newVal?.empresa ?? null,
    form.tipo_anuncio_id = newVal?.empresa_id ?? null
    tipoAnuncioSeleccionado.value = newVal?.tipo_anuncio ?? null,
